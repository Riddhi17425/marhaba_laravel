...
<section class="section_pt section_mb section_gradientbg_product">
    <div class="container-fluid">
        <div class="row">
            <!-- Main Image -->
            <div class="col-md-4">
                <img id="mainBoyImg" src="{{ $mainImage }}" alt="{{ $product->product_detail_name }}" class="img-fluid mb-md-0 mb-4">
            </div>
            ...
            <div class="col-md-8">
                <div class="row">
                    <div class="col-md-10">
                        ...
                        <h2 class="mean_head">{{ $product->name }}</h2>
                    </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Initial selected size & brand (both are selected by default - first of each)
    ...
    function updateContent() {
        ...
        // Hide all items initially and check for matches
        items.forEach(item => {
            const size = item.getAttribute('data-size');
            const brand = item.getAttribute('data-brand');

            // Only show items that match BOTH selected size AND selected brand
        ...
        // Show or hide "Product Not Available" message
        if (hasVisibleItems) {
            noProductMessage.style.display = 'none';
        } else {
            noProductMessage.style.display = 'block';
            // Reset main image to default
            mainImg.style.opacity = 0;
            setTimeout(() => {
                mainImg.src = '{{ $mainImage }}';
                mainImg.alt = '{{ $product->product_detail_name }}';
                mainImg.style.opacity = 1;
            }, 200);
        }

        // Update main image if there's a visible item
        if (firstVisible) {
        ...
        // Update brand logo based on selected brand
        const selectedBrandButton = document.querySelector(`#brandTabs button[data-id="${selectedBrand}"]`);
        const brandImg = selectedBrandButton?.getAttribute('data-brand-img') || '{{ asset('public/brand_images/' . $brandList->first()->image) }}';
        if (brandLogo && brandImg) {
            brandLogo.style.opacity = 0;
            setTimeout(() => {
                brandLogo.src = brandImg;
    ...
    items.forEach(item => {
        ...
        img.addEventListener('click', function() {
            const boyImg = img.getAttribute('data-boy') || img.src;
            const brandImg = item.getAttribute('data-brand-img');
            ...
            // Update content to reflect clicked variant
            updateContent();
        });
    });
});
