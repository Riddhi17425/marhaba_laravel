    ...
    <div class="row align-items-center mb-4">
        <div class="col-12">
            <div class="card-header py-3 no-bg bg-transparent d-flex justify-content-between align-items-center border-bottom flex-wrap">
                <h3 class="fw-bold mb-0">Add Cloth Size </h3><span class="text-danger">NOTE: Need to add Size between below range - <br/>Age Range : 0 Months to 3 Years<br/>Age Range : 2 Years to 7 Years<br/>Age Range : 6 Years to 14 Years</span>
            </div>
        </div>
    </div>

    <div class="card-body">
        <form method="POST" enctype="multipart/form-data" action="{{ route('clothsize.store') }}">
            @csrf
            <div class="card mb-3 p-3">
                <div class="card-header py-3 p-0 bg-transparent border-bottom-0">
                    <h6 class="mb-0 fw-bold">Cloth Size Information</h6>
                </div>
                ...
                {{-- <div class="row g-3 align-items-center mt-2">
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-2">
                                <select name="month_year1" class="form-control @error('month_year1') is-invalid @enderror">
                                    ...
                                    <option value="months" {{ old('month_year1') == 'months' ? 'selected' : '' }}>Months</option>
                                    <option value="years" {{ old('month_year1') == 'years' ? 'selected' : '' }}>Years</option>
                                </select>
                                @error('month_year1')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            ...
                            <div class="col-md-2">
                                <select name="month_year2" class="form-control @error('month_year2') is-invalid @enderror">
                                    <option value="">Select</option>
                                    <option value="months" {{ old('month_year2') == 'months' ? 'selected' : '' }}>Months</option>
                                    <option value="years" {{ old('month_year2') == 'years' ? 'selected' : '' }}>Years</option>
                                </select>
                                @error('month_year2')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                    ...
                    <div class="col-md-6">
                        ...
                        <input type="text" name="name_text" class="form-control @error('name_text') is-invalid @enderror" value="{{ old('name_text') }}" >
                        @error('name_text')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
