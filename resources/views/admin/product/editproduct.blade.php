                                ...
                                <label class="form-label">Brand</label>
                                    <select name="brand_id" class="form-control brand-select">
                                        ...
                                        @foreach($brands as $brand)
                                            <option value="{{ $brand->id }}" data-image="{{ $brand->image }}" {{ old('brand_id', $product->brand_id) == $brand->id ? 'selected' : '' }}>{{ $brand->name }}</option>
                                        @endforeach
                                    </select>
...
$(document).ready(function(){
    ...
    // MAIN BRAND PREVIEW (important fix)
    let mainBrandImage = $('.brand-select').first().find(':selected').data('image');
    if(mainBrandImage && mainBrandImage !== ''){
        $('#brand-preview')
            .attr('src', "{{ asset('public/brand_images') }}/" + mainBrandImage)
            .show();
    }
    updateButtons();
});
